...
predictions.hispec <- read.csv('./calc/fastai/resnet/hispec_predictions.csv')
predictions.cnn <- read.csv('./calc/fastai/resnet/alexnet_hispec_predictions.csv')
...
# RESNET
##########
predictions.hispec$truth <- id2truth[predictions.hispec$ID + 1]
predictions.hispec$model <- 'ResNet Transfer Learning'
cor.data.resnet <- cor.test(
                     x = predictions.hispec$predictions, 
                     y = id2truth[predictions.hispec$ID + 1],
                     method = 'pearson')
...
pROC_obj <- roc(m$labels,m$predictions,
            ...
            plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
            print.auc=TRUE, show.thres=TRUE)
auc.label.hispec <- sprintf(
    "ResNet Transfer Learning: AUC = %.2f (95%% DeLong CI [%.2f - %.2f])",
    pROC_obj$auc,
    pROC_obj$ci[1],
    pROC_obj$ci[3]
)
pROC_obj.hispec <- pROC_obj

roc.test(pROC_obj.hispec, pROC_obj.signalstrength)
...
roc.test(pROC_obj.hispec, pROC_obj)
...
ggsave('./fig/fastai/resnet/multimodel_roc_gradability_cutoff_4.png', height=8, width=8)
...
ggsave('./fig/fastai/resnet/roc_gradability_hispec.png', height=8, width=8)
...
ggsave('./fig/fastai/resnet/roc_gradability_hispec_accuracycutoffs.png', height=8, width=8)
